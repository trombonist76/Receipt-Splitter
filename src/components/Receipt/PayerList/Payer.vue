...
import Button from '@/components/Receipt/Shared/Button.vue';
  ...
  const returnOldName = (event) => {
    if(event.relatedTarget === null && isDifferent){ 
...
<template>
    <div class="flex gap-2.5 mb-2" @focusout.stop="returnOldName" >
      ...
      <Button :disabled="!updatedPayerName || hasError" v-if="isDifferent" @click="changePayerName" styleType="add-btn" icon="plus"/>
